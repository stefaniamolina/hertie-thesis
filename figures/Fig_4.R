...
t7 <- table(lbsample$educ, lbsample$DTNASC) %>%
  prop.table(margin = 2) %>%
  as.data.frame() %>%
  mutate(group = 'Brazil')
...
all$group <- factor(all$group, levels = c("Brazil", "North", "Northeast", "Central-West", 
                                          "South", "Southeast"))


fig4 <- all %>% ggplot(aes(x=Var2, y=(Freq)*100, fill = Var1)) %>%
